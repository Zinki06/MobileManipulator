...
#ifndef MANIPULATION_HARDWARE__OPENCR_HPP_
#define MANIPULATION_HARDWARE__OPENCR_HPP_
...
#include "manipulation_hardware/dynamixel_sdk_wrapper.hpp"
#include "manipulation_hardware/opencr_control_table.hpp"
#include "manipulation_hardware/opencr_definitions.hpp"
...
public:
  ...
  explicit OpenCR(std::unique_ptr<DynamixelSDKWrapper> transport);
  virtual ~OpenCR();
  ...
  bool joints_torque(uint8_t onoff) const;
  bool wheels_torque(uint8_t onoff) const;
  ...
  bool set_gripper_current(int16_t current = opencr::grippers::GOAL_CURRENT);
  bool refresh_goals();
  double get_gripper_goal_position();
  ...
  bool send_heartbeat(const uint8_t & count);

  bool write_byte(const uint16_t & address, uint8_t data);
...
private:
  ...
  int32_t last_gripper_tick_{std::numeric_limits<int32_t>::min()};

  std::mutex buffer_m_;
};
}  // namespace manipulation_hardware
}  // namespace robotis
#endif  // MANIPULATION_HARDWARE__OPENCR_HPP_
