...
OpenCR::OpenCR(std::unique_ptr<DynamixelSDKWrapper> transport)
: dxl_sdk_wrapper_(std::move(transport)) {}

// Lifecycle callbacks own shutdown. Destruction must never move the arm or fingers.
OpenCR::~OpenCR() = default;
...
bool OpenCR::joints_torque(uint8_t onoff) const
{
  return dxl_sdk_wrapper_->write_byte(opencr_control_table.torque_joints.address, onoff);
}

bool OpenCR::wheels_torque(uint8_t onoff) const
{
  return dxl_sdk_wrapper_->write_byte(opencr_control_table.torque_wheels.address, onoff);
}
...
std::array<double, 2> OpenCR::get_wheel_positions()
{
  ...
  positions[opencr::wheels::LEFT] =
    opencr::wheels::TICK_TO_RAD * ticks[opencr::wheels::LEFT];
  positions[opencr::wheels::RIGHT] =
    opencr::wheels::TICK_TO_RAD * ticks[opencr::wheels::RIGHT];
...
std::array<double, 2> OpenCR::get_wheel_velocities()
{
  ...
  velocities[opencr::wheels::LEFT] = opencr::wheels::RAW_VELOCITY_TO_RAD_PER_SEC * rpms[opencr::wheels::LEFT];
  velocities[opencr::wheels::RIGHT] = opencr::wheels::RAW_VELOCITY_TO_RAD_PER_SEC * rpms[opencr::wheels::RIGHT];
...
std::array<double, 4> OpenCR::get_joint_velocities()
{
  ...
  for (uint8_t i = 0; i < rpms.size(); i++) {
    velocities[i] = rpms[i] * 0.229 * opencr::joints::RPM_TO_RAD_PER_SEC;
  }
...
double OpenCR::get_gripper_velocity()
{
  ...
  return rpm * 0.229 * opencr::joints::RPM_TO_RAD_PER_SEC * opencr::grippers::RAD_TO_METER;
}
...
bool OpenCR::set_joint_positions(const std::vector<double> & radians)
{
  ...
  return comm_result && dxl_sdk_wrapper_->write_byte(opencr_control_table.goal_position_write_joints.address, 1);
}
...
bool OpenCR::set_joint_profile_acceleration(const std::array<int32_t, 4> & acceleration)
{
  ...
  return comm_result && dxl_sdk_wrapper_->write_byte(opencr_control_table.profile_acceleration_write_joints.address, 1);
}
...
bool OpenCR::set_joint_profile_velocity(const std::array<int32_t, 4> & velocity)
{
  ...
  return comm_result && dxl_sdk_wrapper_->write_byte(opencr_control_table.profile_velocity_write_joints.address, 1);
}
...
{
  if (!std::isfinite(meters) || meters < -0.010 || meters > 0.019) {return false;}
  ...
  // Heartbeats continue independently; identical goals need no servo bus write.
  if (tick == last_gripper_tick_) {return true;}
  bool comm_result = set_gripper_variables(
    opencr_control_table.goal_position_gripper.address, tick);

  if (!comm_result || !dxl_sdk_wrapper_->write_byte(
      opencr_control_table.goal_position_write_gripper.address, 1)) {return false;}
  last_gripper_tick_ = tick;
  return true;
}

bool OpenCR::refresh_goals()
{
  // Request a servo goal refresh, not another position command.
  return dxl_sdk_wrapper_->write_byte(opencr_control_table.goal_position_read.address, 1);
}

double OpenCR::get_gripper_goal_position()
{
  const auto tick = get_data<int32_t>(opencr_control_table.goal_position_gripper.address, 4);
  return convert_tick_to_radian(tick, opencr::joints::MAX_TICK, opencr::joints::MIN_TICK,
    opencr::joints::MAX_RADIAN, opencr::joints::MIN_RADIAN) * opencr::grippers::RAD_TO_METER;
}
...
bool OpenCR::set_gripper_profile_acceleration(const int32_t & acceleration)
{
  ...
  return comm_result && dxl_sdk_wrapper_->write_byte(opencr_control_table.profile_acceleration_write_gripper.address, 1);
}
...
bool OpenCR::set_gripper_profile_velocity(const int32_t & velocity)
{
  ...
  return comm_result && dxl_sdk_wrapper_->write_byte(opencr_control_table.profile_velocity_write_gripper.address, 1);
}
...
bool OpenCR::set_gripper_current(int16_t current)
{
  union Data {
    int16_t word[1];
    uint8_t byte[1 * 2];
  ...
  if (current <= 0 || current > opencr::grippers::GOAL_CURRENT) {return false;}
  data.word[0] = current;
  ...
  return comm_result && dxl_sdk_wrapper_->write_byte(opencr_control_table.goal_current_write_gripper.address, 1);
}
...
bool OpenCR::write_byte(const uint16_t & address, uint8_t data)
{
  return dxl_sdk_wrapper_->write_byte(address, data);
}

bool OpenCR::send_heartbeat(const uint8_t & count)
{
  return this->write_byte(opencr_control_table.heartbeat.address, count);
}
}  // namespace manipulation_hardware
}  // namespace robotis
