...
"""Start independent OpenCR hardware using read-only reference geometry."""
...
class HardwareDescription(Substitution):
    """Select the independent driver in an in-memory copy of the reference model."""
    ...
    def perform(self, context):
        """Expand the model without writing any reference description or mesh."""
        root = ET.fromstring(self.command.perform(context))
        for hardware in root.findall('./ros2_control/hardware'):
            plugin = hardware.find('plugin')
            if plugin is not None and 'TurtleBot3ManipulationSystem' in (plugin.text or ''):
                plugin.text = 'manipulation_hardware/OpenCRSystem'
                ET.SubElement(hardware, 'param', name='gripper_goal_current_raw').text = '80'
...
def generate_launch_description():
    """Start the independent OpenCR driver and standard ROS controllers."""
    description = get_package_share_directory('turtlebot3_manipulation_description')
    bringup = get_package_share_directory('manipulation_bringup')
    robot = ParameterValue(HardwareDescription(Command([
    ...
    controller = Node(
        ...
        parameters=[{'robot_description': robot}, PerformanceParameters(controller_parameters(
            os.path.join(bringup, 'config', 'controllers.yaml')),
