...
스캔은 45도 간격 8방향, 검출 확인 3~5프레임이며 빈 방향도 5프레임 검사 후
RGB·JSON을 저장한다. 통합 launch에서는 카메라 보정·+30mm·몸통 후보/pitch 선택을
...
한 물체의 depth·재검출·IK·집기 실패는 해당 UUID만 제외하고 나머지 순회를
...
실패하면 전체 임무를 중단한다. 집기 성공 이후 실패한 경로에서는 물체를 쥔 채 다음
스테이션으로 가지 않도록 그리퍼 해제와 팔 파킹을 먼저 수행한다.
