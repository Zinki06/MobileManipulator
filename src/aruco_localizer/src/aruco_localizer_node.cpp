...
public:
    ...
    last_valid_marker_id_(-1)
  {
    ...
    // min_marker_dist is measured on the floor plane in base_link, not as
    // the camera-to-marker 3D slant range.
    this->declare_parameter<double>("min_marker_dist", 0.40);
    this->declare_parameter<double>("max_marker_dist", 2.50);
    this->declare_parameter<double>("max_correction_angular_speed", 0.15);
    this->declare_parameter<double>("max_dead_reckoning_time", 120.0);
    this->declare_parameter<double>("max_dead_reckoning_distance", 8.0);
    this->declare_parameter<double>("max_translation_correction_step", 0.03);
    this->declare_parameter<double>("max_yaw_correction_step", 0.03);
    ...
    max_correction_angular_speed_ =
      this->get_parameter("max_correction_angular_speed").as_double();
    max_dead_reckoning_time_ =
      this->get_parameter("max_dead_reckoning_time").as_double();
    max_dead_reckoning_distance_ =
      this->get_parameter("max_dead_reckoning_distance").as_double();
    max_translation_correction_step_ =
      this->get_parameter("max_translation_correction_step").as_double();
    max_yaw_correction_step_ =
      this->get_parameter("max_yaw_correction_step").as_double();

    if (filter_alpha_ <= 0.0 || filter_alpha_ > 1.0 ||
      marker_freshness_timeout_ <= 0.0 ||
      relocalization_consistency_count_ < 1 ||
      min_marker_dist_ < 0.0 || max_marker_dist_ <= min_marker_dist_ ||
      max_correction_angular_speed_ <= 0.0 ||
      max_dead_reckoning_time_ <= 0.0 ||
      max_dead_reckoning_distance_ <= 0.0 ||
      max_translation_correction_step_ <= 0.0 ||
      max_yaw_correction_step_ <= 0.0)
    {
      throw std::invalid_argument("Invalid ArUco localization parameter range.");
    }
    ...
    localization_initialized_pub_ = this->create_publisher<std_msgs::msg::Bool>(
      "/aruco/localization_initialized", 10);
    localization_mode_pub_ = this->create_publisher<std_msgs::msg::String>(
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Aruco Localization Node Initialized. (EMA Alpha: %.2f, "
      "Min planar range: %.2fm, Max correction yaw rate: %.2frad/s)",
      filter_alpha_, min_marker_dist_, max_correction_angular_speed_
    );
  }
...
private:
  ...
  void odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
  {
    ...
    if (has_last_odom_position_) {
      const double increment = (current_position - last_odom_position_).norm();
      // Odometry resets or corrupt samples must not immediately degrade localization.
      if (std::isfinite(increment) && increment < 1.0) {
        distance_since_correction_ += increment;
      }
    }
    last_odom_position_ = current_position;
    has_last_odom_position_ = true;
    latest_odom_angular_speed_ = msg->twist.twist.angular.z;
  }
  ...
  void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg)
  {
    ...
    for (size_t i = 0; i < marker_ids.size(); ++i) {
      ...
      cv::aruco::estimatePoseSingleMarkers(
        single_corner, marker.size, camera_matrix_, dist_coeffs_,
        rvecs, tvecs);

      const double dist = std::sqrt(
        tvecs[0][0] * tvecs[0][0] + tvecs[0][1] * tvecs[0][1] +
        tvecs[0][2] * tvecs[0][2]);

      // The optical 3D distance includes camera height, so it cannot
      // protect a floor marker at a 25cm horizontal offset. Transform
      // the marker origin into base_link and gate on floor-plane range.
      const Eigen::Vector3d marker_in_camera(
        tvecs[0][0], tvecs[0][1], tvecs[0][2]);
      const Eigen::Vector3d marker_in_base =
        T_cam_to_base.inverse() * marker_in_camera;
      const double planar_dist = std::hypot(
        marker_in_base.x(), marker_in_base.y());
      const aruco_localizer::MarkerCorrectionPolicy correction_policy{
        min_marker_dist_, max_marker_dist_, max_correction_angular_speed_};
      ...
      if (correction_decision != aruco_localizer::CorrectionDecision::ACCEPT) {
        RCLCPP_INFO_THROTTLE(
          ...
          "[Correction Frozen] reason=%s marker=%d planar=%.3fm "
          "spatial=%.3fm angular=%.3frad/s",
          aruco_localizer::correctionDecisionName(correction_decision),
          id, planar_dist, dist, latest_odom_angular_speed_);
        continue;
      }
      ...
      double odom_yaw = std::atan2(
        T_map_to_odom_single.rotation()(
          1,
          0),
        T_map_to_odom_single.rotation()(0, 0));
    ...
    if (valid_marker_count == 0 || total_weight <= 0.0) {
      relocalization_candidate_count_ = 0;
      return;
    }
    ...
    if (!has_valid_map_odom_) {
      ...
      relocalization_candidate_count_ = 0;
      RCLCPP_INFO(
        this->get_logger(), "[Initial Lock] Map->Odom initialized: (%.3f, %.3f, %.1f°)",
    ...
    distance_since_correction_ = 0.0;
  ...
  void publishLocalizationStatus(bool fresh)
  {
    ...
    initialized_msg.data = has_valid_map_odom_;
    localization_initialized_pub_->publish(initialized_msg);

    const double correction_age = has_valid_map_odom_ ?
      (this->now() - last_valid_marker_time_).seconds() : -1.0;
    const bool degraded = has_valid_map_odom_ &&
      (correction_age > max_dead_reckoning_time_ ||
      distance_since_correction_ > max_dead_reckoning_distance_);
    ...
    if (!has_valid_map_odom_) {
      ...
      mode_msg.data = "DEAD_RECKONING";
    }
    localization_mode_pub_->publish(mode_msg);

    std_msgs::msg::Float64 correction_age_msg;
    correction_age_msg.data = correction_age;
    correction_age_pub_->publish(correction_age_msg);

    std_msgs::msg::Float64 distance_msg;
    distance_msg.data = distance_since_correction_;
    distance_since_correction_pub_->publish(distance_msg);
  ...
  double max_correction_angular_speed_;
  double max_dead_reckoning_time_;
  double max_dead_reckoning_distance_;
  double max_translation_correction_step_;
  double max_yaw_correction_step_;
  ...
  Eigen::Vector2d last_odom_position_;
