...
def test_collision_timeout_speed_and_map_jump(tmp_path, monkeypatch):
    ...
    processes = [subprocess.Popen([str(guard)], stdout=outputs[0], stderr=subprocess.STDOUT),
    ...
    def publish():
        ...
        mode.publish(String(data='MARKER_CORRECTED'))
        scan = LaserScan()
    ...
    try:
        ...
        assert all(0.0 <= v <= 0.10001 for _, v in received)
        state['pivot'] = True
        time.sleep(0.5)
